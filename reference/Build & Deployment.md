...
**Agent-facing.** The package/dependency inventory and the container strategy for everything that gets built and deployed. Pairs with [[AWS Stack]] (what runs where), [[Security]] (CVE/secret scanning gates), and [[Design principles]] (12-factor, DRY). This doc is current-state only, same convention as `wiki/` — see `AGENTS.md`.

## State
No application source exists yet (`backend/app`, `frontend/src`, `infra/lib` are not created). What exists now: `backend/pyproject.toml`, `frontend/package.json`, `infra/package.json` (full dependency manifests) and `docker/*.Dockerfile` + `docker-compose.yml` (the build/container definitions). The Dockerfiles `COPY` from source directories that don't exist yet and will not build successfully until the FastAPI app, React app, and CDK stacks are scaffolded — that's expected, not a bug in these files.

## Why Docker at all here
Nothing in this app runs as a long-lived container in production — compute is Lambda, the frontend is a static S3/CloudFront bundle (see [[AWS Stack]]). Docker is used for two distinct jobs, and it's worth keeping them mentally separate:
...
| Package | Why |
|---|---|
| `fastapi` | app framework, carried over per [[AWS Stack]] |
| `pydantic` v2 | request/response schemas |
| `pydantic-settings` | env-based config, 12-factor per [[Design principles]] |
...
| `aws-lambda-powertools` | structured JSON logging to stdout (observability requirement in [[Design principles]]), plus its **Idempotency** utility — this *is* the DynamoDB idempotency-key mechanism [[AWS Stack]] specifies for the ingestion pipeline, not a hand-rolled table check |
| `python-jose[cryptography]` | verifies Cognito-issued JWTs server-side on every request — [[Security]] requires this never be trusted from client claims alone |
...
| `pip-audit` | CVE scan in CI, blocks merge on a hit, per [[Security]] |
| `pre-commit` | runs lint/format before a commit reaches an agent's context, per `reference/Usage principals.md` |
...
| Package | Why |
|---|---|
| `react`, `react-dom` | carried over per [[AWS Stack]] |
...
| `zustand` | minimal client-only state (compose draft, mention-autocomplete UI state) — deliberately not Redux, KISS per [[Design principles]] |
...
| Package | Why |
|---|---|
| `vite`, `@vitejs/plugin-react` | build tool, per [[AWS Stack]] |
...
## CI/CD wiring
GitHub Actions (OIDC-federated role, no long-lived keys, per [[AWS Stack]]):
- On PR: build `docker/backend.Dockerfile` target `test` and `docker/frontend.Dockerfile` target `test`, run both — this is `reference/Usage principals.md` category 3 (scripted execution), results distilled to `reports/test-runs/`, not fed raw into any interactive agent's context.
...
- `pip-audit` and `npm audit`/Dependabot run in CI per [[Security]] and block merge on an unpatched critical.

...
| A task spec that is impossible to satisfy — `run_context_maintainer` on, with no wiki module in `allowed_paths`. Escalated a task whose code and CI were already green | **Closed (#35)**: `sp.validate` rejects it, and `selfcheck` validates committed specs, so CI catches it too | — |
| A human branch cut from an agent branch, carrying `[agent-state]` commits and mid-flight task state onto `main` | **Partly closed (#36)**: a `pre-push` hook refuses it | A hook is a convenience, not a control. The real gate would be a CI check that a non-`agent/*` PR's diff touches no `.ai/tasks/*/state.json` — deferred only to avoid colliding with in-flight CI work |

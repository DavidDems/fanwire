...
5. **Two repository secrets.** `ANTHROPIC_API_KEY` for the agent seam, and
   ...
   or expired key degrades rather than breaks. Set
   `decisions.<point>.enabled: false` in `config.json` for the same effect on
   one decision — it runs the identical fallback path, so "turned off" and

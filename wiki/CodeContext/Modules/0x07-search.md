...
- ~~**Exact `player_stats` field name for "position"**~~ — **Resolved: `"position"`.** Implemented in `ApiSportsAdapter._to_normalized_game` ([[0x02-events]] "Resolved decisions"), illustrative/unverified against a live API-SPORTS response like the rest of that adapter's vendor-shape parsing — the filter query's JSONB path can target `player_stats[*].position`.

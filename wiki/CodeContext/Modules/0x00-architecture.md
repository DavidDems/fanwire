...
- **No production migration runner exists yet** — narrowed, not closed. `docker/backend.Dockerfile`'s `lambda` target now copies `backend/alembic/` and `alembic.ini`, so the image *carries* the migrations (that was the half `INFRA-003` was blocked on, and it has landed). Nothing in the deploy path runs `alembic upgrade head`, so a real deploy still ships schema changes with no migration step. `INFRA-003` is the task that closes it: a migration handler and a Lambda over the same image, invoked by a human and by nothing else.

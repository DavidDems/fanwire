...
## Module scope
`posts/` owns four tables: `Post`, `PostLike`, `EventMention`, `Report`. Covers every "Posts" business rule in [[reference/Projects|Projects]]: create/reply/repost, like, image attachment, game-result attachment, report (flag only), copy-link (client-side, no schema impact), and code-injection safety.
...
- No `updated_at` / edit support — no "edit a post" business rule exists in [[reference/Projects|Projects]], so a published `Post` is treated as immutable (see Design principles below).

**Design principles** ([[reference/Design principles|Design principles]])
...
**Pattern tie-in** ([[reference/Gang of Four Example|Gang of Four Example]])
...
- **Rate limiting**: posting is rate-limited per user via API Gateway usage plan + a DynamoDB cache check, per Security.md's user-generated-content section.

**Open decisions**
- Whether a "quote repost" (repost + own commentary) needs its own flag, or is just `is_repost=true` with non-empty `text` — [[reference/Projects|Projects]] only says "repost," doesn't distinguish.
...
**Security** — no free-text input, so no injection surface. [[reference/Security|Security]] only explicitly calls out rate-limiting for posting and reporting, not liking — see Open decisions.
...
- Whether like-spam needs its own rate limit — Security.md doesn't mention it explicitly; flagging rather than assuming it's out of scope.

## EventMention
Join table linking a `Post` to a `Game` row. `Game` is owned by `events/` — see [[0x02-events]], not redefined here. This is the schema behind "have a sports game result as a part of a post/reply" in [[reference/Projects|Projects]].
...
**Design principles**
- **Dependency Inversion / connection rule** — `posts/` references `events.Game` only by FK/ID, never by importing `events/`'s internal classes, matching [[0x00-architecture]]'s connection rule and [[reference/Gang of Four Example|Gang of Four Example]]'s module boundary.
...
**Open decisions**
- What happens when `MentionParser` extracts a token that doesn't resolve to any known `Game`/team (game not yet ingested, typo, etc.) — silently dropped, stored as an unresolved placeholder, or rejects the whole post? Not specified in [[reference/Projects|Projects]].
- Whether there's a per-post cap on mention count (spam/abuse vector via mention flooding) — not specified.

## Report
The **only** reporting-related table for v1, per [[reference/Projects|Projects]]'s closing line ("no moderation or reporting for now, only a reported flag on posts with a table for all reported cases"). No justification text field, no status/workflow field — see [[0x00-architecture]] for the broader "moderation chain not built for v1" statement.
...
**Design principles**
- **Idempotency** — the unique `(post_id, reporter_id)` constraint makes reporting the same post twice safe to retry, matching [[reference/Design principles|Design principles]]'s idempotency rule.
...
**Pattern tie-in**
- None of Composite/Interpreter/Facade apply. Note explicitly: [[reference/Gang of Four Example|Gang of Four Example]]'s `ReportPostCommand` (with `execute()`/`undo()` for a moderation audit trail) is **not implemented** — there's no moderation flow for it to undo into. `posts/` still publishes `PostReported` on the `PostEventBus` per [[0x00-architecture]]'s connection rule, but for v1 no `moderation/`/`reporting/` subscriber exists to consume it.

**AWS mapping** — RDS Postgres, table `reports`.

**Security**
- **Rate limiting** — reporting is rate-limited per user via API Gateway usage plan + DynamoDB cache check, same mechanism and same Security.md line as post rate limiting.
- **No moderation workflow, by design** — this is a v1 scope boundary, not an oversight; re-confirm against [[reference/Projects|Projects]] before adding any status/review field.

**Open decisions**
- No admin/query surface is defined yet for anyone to actually look at accumulated `reports` rows — the table exists per the business rule, but nothing in [[reference/Projects|Projects]] or this wiki specifies who reads it or how.

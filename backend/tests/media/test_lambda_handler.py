...
garbage bytes) so validate_type always succeeds -- isolating the behavior
under test (GuardDutyScanResultScanner driving scan_for_malware's verdict)
from validate_type's own unrelated rejection paths, which
tests/media/test_pipeline.py already covers.
"""
...
def test_threats_found_scan_result_rejects_and_deletes_the_object(session_factory, s3_and_media):
